...
export function QuestionCard({
    ...
    return (
        <section
            className="rounded-(--radius-lg) border border-(--border) bg-(--surface) p-4 shadow-(--shadow-sm) sm:p-5"
            aria-labelledby={`question-${question.id}-title`}
        >
            {imageUrl ? (
                <div className="mb-4 overflow-hidden rounded-(--radius-md) border border-(--border) bg-(--surface-muted)">
                ...
                {question.options.map((option) => {
                    ...
                    return (
                        <label
                            key={option.id}
                            className={[
                                'flex min-h-11 cursor-pointer items-center gap-3 rounded-(--radius-md) border p-3 transition',
